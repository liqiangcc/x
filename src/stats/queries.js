...
function assertMetricColumn(columnName) {
  if (!/^c\d+$/.test(String(columnName ?? ""))) {
    throw new Error(`Invalid metric column: ${columnName}`);
  }
  return columnName;
}

function buildYearlyPositiveQuery({ metricColumn, stockCode = null } = {}) {
  const normalizedMetricColumn = assertMetricColumn(metricColumn);
...
WITH Changes AS (
  SELECT
    c1,
    CASE
      WHEN LAG(${normalizedMetricColumn}, 1, 0) OVER (PARTITION BY c12 ORDER BY c1) = 0 THEN NULL
      ELSE (${normalizedMetricColumn} * 100.0 / LAG(${normalizedMetricColumn}, 1, 0) OVER (PARTITION BY c12 ORDER BY c1)) - 100.0
...
function buildNewHighsQuery({ year = null, date = null } = {}) {
  if (year && date) {
...
`,
  ...
  const selectClause = year
    ? "c1 AS Date, COUNT(*) AS BreakoutCount"
    : "SUBSTR(c1, 1, 4) AS Year, COUNT(*) AS BreakoutCount";
