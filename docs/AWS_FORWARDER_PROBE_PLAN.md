...
第一版先部署 3 到 5 个常用 Region，不要一口气部署全部 17 个 Region。
...
最多 fallback ROUTER_MAX_FALLBACKS 个 region，默认 3
所有 region 失败返回 502，并包含 attempted_regions
```

Router 调 Target 使用 AWS SDK `InvokeFunction`。每个 Target invoke 必须有 `ROUTER_TARGET_TIMEOUT_MS` 超时控制，默认 3000ms。
...
```text
schedule 或 engine 为空或 engine=aws 时配置 AWS credentials
engine=aws-router 时不配置 AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY
```
...
默认 engine 仍保持 aws，除非后续 A/B 测试证明 aws-router 更稳
```
...
如果 `aws-router` 明显更稳，再单独讨论是否调整 GitHub Action 默认 engine。不要在 v1 自动替换 `auto`。
...
- 单 region invoke timeout 默认 3000ms。
