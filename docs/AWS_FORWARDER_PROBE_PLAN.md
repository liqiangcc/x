...
第一版先部署 3 到 6 个常用 Region，不要一口气部署全部 17 个 Region。`ap-northeast-1` 可作为 Router Region，也可作为第一 Target；如果单独访问东财不稳定，再通过 `--target-regions` 调整顺序。
...
最多尝试 ROUTER_MAX_FALLBACKS 个 region，默认 6
