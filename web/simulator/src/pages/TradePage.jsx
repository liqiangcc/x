...
export default function TradePage() {
  ...
  const visibleCandidate = chart ?? candidate;
  const currentPosition = portfolio?.positions?.find((position) => position.candidateId === candidate?.candidateId) ?? null;

  function openOrderEditor(side) {
    setSellTarget(side === "sell" ? currentPosition : null);
    window.setTimeout(() => document.getElementById("order-editor")?.scrollIntoView({ behavior: "smooth", block: "center" }), 0);
  }

  async function addCurrentToWatchlist() {
    if (!candidate || chart?.detail?.isWatchlisted) return;
    await run(() => client.addWatchlist(sessionId, candidate.candidateId));
    setChart((current) => {
      if (!current) return current;
      const next = { ...current, detail: { ...current.detail, canBuy: true, isWatchlisted: true } };
      chartCache.set(`${sessionId}:${candidate.candidateId}:${session?.dayIndex ?? 1}`, next);
      return next;
    });
  }
  ...
  return (
    <section className="stock-detail-page">
      <div className="stock-quote-header">
        <div className="stock-quote-identity"><p className="eyebrow">{tradingDayLabel({ anonymousMode: settings.anonymousMode, date: session.clock.currentDate, dayIndex: session.dayIndex ?? 1 })} · D+1 开盘执行</p><h1>{visibleCandidate ? securityLabel(visibleCandidate, settings.anonymousMode) : "当日无候选"}</h1><span className="data-badge">近似价格</span></div>
      ...
      <div className="trade-workspace">
        <div className="trade-chart-grid">
          <div className="panel chart-panel mobile-active"><header><h2>{mobileTab === "daily" ? "日K走势" : "年K走势"}</h2><span>拖动缩放 · 长按查看开高低收</span></header>{chart ? mobileTab === "daily" ? <DailyChart anonymousMode={settings.anonymousMode} rows={chart.daily} showBoll={showBoll} showVolume={showVolume} /> : <YearlyChart anonymousMode={settings.anonymousMode} rows={chart.yearly} /> : <div className="chart-loading">{candidate ? "加载图表…" : "当日没有买入候选，仍可管理已有持仓。"}</div>}{chart && <div className="stock-chart-actions"><button className={chart.detail?.isWatchlisted ? "watchlist-action active" : "watchlist-action"} disabled={busy || chart.detail?.isWatchlisted} onClick={addCurrentToWatchlist} type="button">{chart.detail?.isWatchlisted ? "★ 已自选" : "☆ 加自选"}</button><button className="buy-action" disabled={busy || session.status !== "waiting_for_decision" || !chart.detail?.canBuy} onClick={() => openOrderEditor("buy")} type="button">买入</button><button className="sell-action" disabled={busy || session.status !== "waiting_for_decision" || !currentPosition || currentPosition.availableQuantity < 1} onClick={() => openOrderEditor("sell")} type="button">卖出</button></div>}</div>
        </div>
        <aside className="panel trade-side-panel">
          <PortfolioPanel anonymousMode={settings.anonymousMode} onSell={session.status === "waiting_for_decision" ? setSellTarget : null} portfolio={portfolio} />
          {session.status === "waiting_for_decision" && (sellTarget || (candidate && chart?.detail?.canBuy)) && <OrderEditor candidate={{ ...(sellTarget ?? candidate), alias: securityLabel(sellTarget ?? visibleCandidate, settings.anonymousMode) }} disabled={busy} estimatedPrice={sellTarget?.averageCost ?? estimatedPrice} initialAmount={settings.defaultBuyAmount} initialQuantity={sellTarget?.availableQuantity ?? 100} initialReason={settings.defaultBuyReason} initialSide={sellTarget ? "sell" : "buy"} maxQuantity={sellTarget?.availableQuantity} onSubmit={submitOrder} />}
