...
import { accountLabel, securityLabel, tradingDayLabel } from "../utils/securityDisplay.js";
...
export default function WatchlistPage() {
  ...
  const label = (item) => securityLabel(item, settings.anonymousMode);
  ...
  return <section><p className="eyebrow">跨日期保留 · {settings.anonymousMode ? "匿名观察" : "实名观察"}</p><div className="watchlist-heading"><h1>我的自选</h1><div className="watchlist-clock"><strong>{accountLabel(session, settings.anonymousMode)}</strong><span>{tradingDayLabel({ anonymousMode: settings.anonymousMode, date: session.clock.currentDate, dayIndex: session.dayIndex ?? 1 })}</span><button className="primary-button" disabled={busy || !session.clock.nextDate} onClick={advance}>{busy ? "推进中…" : "下一交易日"}</button></div></div><ErrorNotice error={error} />
    ...
    <div className="watchlist-list">{visibleItems.map((item) => {
      const detailPath = `/accounts/${sessionId}/stocks/${item.candidateId}`;
      return <article aria-label={`查看${label(item)}详情`} className="watchlist-row watchlist-row-link" key={item.candidateId} onClick={() => navigate(detailPath)} onKeyDown={(event) => {
        if (event.target !== event.currentTarget || (event.key !== "Enter" && event.key !== " ")) return;
        event.preventDefault();
        navigate(detailPath);
      }} role="link" tabIndex={0}><div className="watchlist-identity"><h2>{label(item)}</h2>{item.signal?.source === "inferred" && <span>推定</span>}</div><button aria-label={`操作${label(item)}`} className="secondary-button compact-button watchlist-menu-button" onClick={(event) => { event.stopPropagation(); setActionTarget(item); }} type="button">操作</button><WatchlistSummary detail={item.detail} /></article>;
    })}</div>
    ...
    {actionTarget && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setActionTarget(null); }}><div aria-label={`${label(actionTarget)}操作`} className="panel action-modal" role="dialog"><div className="modal-heading"><strong>{label(actionTarget)}</strong><button className="text-button" onClick={() => setActionTarget(null)} type="button">关闭</button></div><div className="action-modal-buttons"><Link className="secondary-button" onClick={() => setActionTarget(null)} to={`/accounts/${sessionId}/stocks/${actionTarget.candidateId}`}>查看走势</Link><button className="primary-button" disabled={busy || session.status !== "waiting_for_decision"} onClick={() => { setTradeTarget({ item: actionTarget, side: "buy" }); setActionTarget(null); }} type="button">买入</button>{actionTarget.detail?.holding && <button className="secondary-button" disabled={busy || session.status !== "waiting_for_decision" || actionTarget.detail.holding.availableQuantity < 1} onClick={() => { setTradeTarget({ item: actionTarget, side: "sell" }); setActionTarget(null); }} type="button">卖出</button>}<button className="text-button danger-button" onClick={async () => { const item = actionTarget; setActionTarget(null); await run(() => client.removeWatchlist(sessionId, item.candidateId)); await reload(); }} type="button">移除自选</button></div></div></div>}
    {tradeTarget && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setTradeTarget(null); }}><div aria-label={`${label(tradeTarget.item)}${tradeTarget.side === "buy" ? "买入" : "卖出"}`} className="panel trade-modal" role="dialog"><div className="modal-heading"><strong>{label(tradeTarget.item)} · {tradeTarget.side === "buy" ? "买入" : "卖出"}{tradeTarget.sellPrompt ? `（收益率 ${tradeTarget.item.detail.holding.unrealizedPnlPct >= 0 ? "+" : ""}${tradeTarget.item.detail.holding.unrealizedPnlPct.toFixed(2)}%，达到止盈提示）` : ""}</strong><button className="text-button" onClick={() => setTradeTarget(null)} type="button">关闭</button></div><OrderEditor candidate={{ ...tradeTarget.item, alias: label(tradeTarget.item) }} disabled={busy} estimatedPrice={tradeTarget.item.detail.currentClose ?? tradeTarget.item.detail.holding?.averageCost ?? 0} initialAmount={settings.defaultBuyAmount} initialQuantity={tradeTarget.side === "sell" ? tradeTarget.item.detail.holding.availableQuantity : 100} initialReason={settings.defaultBuyReason} initialSide={tradeTarget.side} maxQuantity={tradeTarget.side === "sell" ? tradeTarget.item.detail.holding.availableQuantity : undefined} onSubmit={submitOrder} /></div></div>}
  </section>;
}
