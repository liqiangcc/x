...
export default function CandidatesPage() {
  ...
  const { busy, client, error, run, session, sessionId, setSelectedCandidate } = useSession();
  ...
  const items = [...(result?.pagination?.items ?? [])].sort(priceAscending);
  ...
  return <section><div className="page-heading-row"><div><p className="eyebrow">当前决策日 · 索引查询</p><h1>匿名候选池</h1></div><span className="data-badge">第 {session.dayIndex ?? 1} 个交易日</span></div><ErrorNotice error={error} />
    <div className="panel result-toolbar"><label>策略<select value={strategyId} onChange={(event) => selectStrategy(event.target.value)}>{strategies.filter((strategy) => !strategy.archived).map((strategy) => <option disabled={strategy.status !== "ready"} key={strategy.id} value={strategy.id}>{strategy.name}</option>)}</select></label>{items.length > 0 && <button className="secondary-button" onClick={() => run(() => client.addWatchlistBulk(sessionId, items.map((item) => item.candidateId)))}>全部加入自选</button>}</div>
    {!result?.calculated && <div className="empty-state"><strong>策略索引尚未就绪</strong></div>}
    <div className="candidate-list">{items.map((candidate) => <CandidateCard candidate={candidate} key={candidate.candidateId} onAdd={() => add(candidate.candidateId)} onSelect={open} />)}</div>
