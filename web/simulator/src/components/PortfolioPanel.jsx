...
export default function PortfolioPanel({ anonymousMode = true, onSell, portfolio }) {
  ...
  return (
    <div className="portfolio-panel">
      ...
      {portfolio.positions?.length > 0 && <div className="position-list">{portfolio.positions.map((position) => <div className="position-row" key={position.candidateId}><strong>{securityLabel(position, anonymousMode)}</strong><span>{position.quantity} 股 · 可卖 {position.availableQuantity}</span><span>成本 ¥{money(position.averageCost)} · 当前 ¥{money(position.currentPrice)}</span><span className={(position.unrealizedPnl ?? 0) < 0 ? "loss-text" : "positive-text"}>持仓收益 {position.unrealizedPnl >= 0 ? "+" : ""}¥{money(position.unrealizedPnl)} · {position.unrealizedPnlPct >= 0 ? "+" : ""}{Number.isFinite(position.unrealizedPnlPct) ? position.unrealizedPnlPct.toFixed(2) : "—"}%</span><span>持仓 {position.holdingDays ?? "—"} 个交易日{(position.priceDayOffset ?? 0) > 0 ? " · 最近可用收盘" : ""}</span>{onSell && <button className="text-button" disabled={position.availableQuantity < 1} onClick={() => onSell(position)} type="button">{position.availableQuantity > 0 ? "卖出" : "T+1 后可卖"}</button>}</div>)}</div>}
    </div>
  );
}
